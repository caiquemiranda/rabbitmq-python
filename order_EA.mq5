...
void OnTimer()
{
   ...
   if (mq.ReceiveMessage(RabbitMQ_Queue, receivedMessage))
   {
      ...
      // Interprete a mensagem e execute a ordem correspondente
      if (StringFind(receivedMessage, "comprar") >= 0)
      {
         ExecuteOrder(OP_BUY);
      }
      else if (StringFind(receivedMessage, "vender") >= 0)
      {
...
//+------------------------------------------------------------------+
//| Função para executar uma ordem de compra ou venda                |
//+------------------------------------------------------------------+
void ExecuteOrder(int orderType)
{
   ...
   {
      OrderSend("EURUSD", OP_SELL, lotSize, price, 2, 0, 0, "", 0, clrNONE);
      Print("Ordem de venda executada: ", lotSize, " lotes a ", price);
   }
}
